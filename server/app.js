...
async function adminByName(name){requirePersistence();if(!USE_POSTGRES){const a=dbRead().admin;return a&&a.username===name?a:null;}await ready();const r=await sql`SELECT id,username,password_hash,created_at FROM admins WHERE username=${name} LIMIT 1`;return r[0]||null;}
...
app.get('/api/health',async(_req,res)=>{try{await settings();res.json({ok:true,database:USE_POSTGRES?'postgres':'local-json',storage:STORAGE_MODE,node:process.version});}catch(e){res.status(e.statusCode||500).json({ok:false,database:PERSISTENCE_MODE,storage:STORAGE_MODE,message:e.message});}});
