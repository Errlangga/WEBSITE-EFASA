...
function resolveBlobMedia(body,kind){
  ...
  // Accept the stable application proxy generated by our upload endpoint.
  if(!pathname && supplied.startsWith('/api/media?path=')){
    try{
      const parsed=new URL(supplied,'https://efasa.local');
      pathname=clean(parsed.searchParams.get('path')||'',1000);
      while(pathname.startsWith('/'))pathname=pathname.slice(1);
    }catch{}
  }

  // Accept the canonical Blob URL returned by the presigned PUT response.
  if(!pathname && supplied){
    try{
      ...
      if(validHost){
        pathname=decodeURIComponent(u.pathname).slice(1);
      }
    }catch{}
  }
